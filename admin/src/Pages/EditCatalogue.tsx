...
export default function EditCatalogue() {
  ...
  return (
    <HomeLayout pageName="Edit Catalogue">
      <div className="space-y-8 overflow-hidden relative max-w-[50rem] mx-auto bg-[#010101] pb-0 border border-[#3c3c3c] p-4 sm:p-6 lg:p-8 py-8 rounded">
        ...
        {currentStep === 1 && (
          <EditOwner
            catalogueOwner={catalogueData?.data?.catalogueOwner}
        ...
        {currentStep === 4 && (
          <EditMetaDetails
            metaDetails={
              catalogueData?.data?.metaDetails as z.infer<
                typeof addMetaDetailsSchema
              >
            }
            catalogueId={catalogueData?.data?._id ?? ""}
            setCurrentStep={setCurrentStep}
            stepsLength={steps.length}
            currentStep={currentStep}
          />
