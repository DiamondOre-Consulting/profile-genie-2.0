...
const AllCatalogue = () => {
  ...
  const { data, isLoading, refetch } = useGetAllCataloguesQuery({
    search: debouncedSearchValue,
    filter: filterValue,
  });
  const [deleteModalActive, setDeleteModalActive] = useState(false);
  const [recycleId, setRecycleId] = useState("");
  const [updateActiveStatus, { isLoading: activeLoading }] =
    useUpdateActiveStatusMutation();
  const [updatePaidStatus, { isLoading: paidLoading }] =
    useUpdatePaidStatusMutation();
  const [recycleCatalogue, { isLoading: recycleLoading }] =
    useRecycleCatalogueMutation();
  ...
  return (
    <HomeLayout pageName="All Catalogue">
                  ...
                  <div className="relative w-full p-3 border shadow-md rounded-xl border-white/20 bg-gradient-to-b from-gray-900 via-gray-950 to-black">
                          ...
                          <div className="flex flex-col items-start">
                            <div className="flex items-center gap-2">
                              <h2 className="text-base font-bold leading-4 text-white">
                                {
                                  item?.ownerDetails?.authAccountDetails
                                    ?.fullName
                                }
                              </h2>
                              <div className="h-1.5 w-1.5 rounded-full bg-green-500 shadow-lg shadow-green-500/50"></div>
                            </div>
                            <p className="text-neutral-300 text-sm w-[10rem] truncate">
                              {item?.name}
                            </p>
                          </div>
                        </div>
                          ...
                          <DropdownMenu>
                            ...
                            <DropdownMenuContent className="max-w-38">
                              ...
                              <DropdownMenuGroup>
                                <DropdownMenuItem
                                  onClick={() =>
                                    handleUpdateStatus(item?._id, "isActive")
                                  }
                                  className="cursor-pointer hover:bg-neutral-950"
                                >
                                  <BoltIcon
                                    size={16}
                                ...
                                {!item.isPaid && (
                                  <DropdownMenuItem
                                    onClick={() =>
                                      handleUpdateStatus(item?._id, "isPaid")
                                    }
                                    className="cursor-pointer hover:bg-neutral-950"
                                  >
                                    <Layers2Icon
                      ...
                      <div className="flex flex-col justify-between gap-2 sm:flex-row md:flex-col">
                        ...
                        >
                          ...
                          <span className="text-sm font-semibold">
                            {item?.ownerDetails?.authAccountDetails?.email}
                          </span>
                        </div>
                        <div
                          onClick={() =>
                        ...
                        >
                          <div className="flex items-center gap-2">
                            ...
                            <span className="text-sm font-semibold">
                              + {item.ownerDetails?.phoneList[0]?.phone}
                            </span>
                          </div>
                        </div>
                      </div>
                        ...
                        <div className="flex items-center justify-between w-full gap-1">
                          ...
                          <span className="text-sm font-semibold text-white">
                            {new Intl.DateTimeFormat("en-US", {
                              ...
                              new Date(
                                new Date(item?.paidDate).setFullYear(
                                  new Date(item?.paidDate).getFullYear() + 1
                                )
                              )
                            )}
                          </span>
                    ...
                    <div className="flex items-center gap-4 mt-4 justify-evenly ">
                      ...
                      <div
                        onClick={() =>
                          window.open(
                            `https://profilegenie.in/catalogue/1/${item?.userName}`
                          )
                        }
                        className="relative flex items-center justify-center cursor-pointer size-10 group/inner"
                      >
                      ...
                      <div
                        onClick={() =>
                          navigate(`/edit-catalogue/${item?.userName}`)
                        }
                        className="relative flex items-center justify-center cursor-pointer size-10 group/inner"
                      >
                        <div className="absolute inset-0 transition-transform duration-1000 border rounded-full border-amber-400/20 border-t-purple-400 group-hover/inner:rotate-180"></div>
