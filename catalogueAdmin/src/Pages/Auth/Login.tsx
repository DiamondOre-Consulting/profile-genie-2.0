...
import { loginValidation, loginValidationSchema } from '@/validations/AuthValidation'
...
const Login = () => {
    ...
    const { register, handleSubmit, formState: { errors, isSubmitting } } = useForm<loginValidation>({
    ...
    const handleGoogleLogin = async () => {
        try {
            window.location.href = "https://server.profilegenie.in/api/v1/auth/google";
        } catch (err) {
            console.log(err);
        }
    }
                        ...
                        <div className="p-5">
                            ...
                            <form noValidate className="w-full" onSubmit={handleSubmit(handleLogin)}>
                                ...
                                <button disabled={isSubmitting} type="submit"
                                    ...
                                    {isSubmitting ? <IconFidgetSpinner className='animate-spin w-4.5' /> : 'LOGIN'}</button>
                            </form>
                        </div>
                    </div>
                </div>
