...
import { Copy, PlusIcon, TimerIcon } from "lucide-react";
...
const TemplateEditor = () => {
  ...
  const [createAuthenticationTemplate] =
    useCreateAuthenticationTemplateMutation();
  ...
  const onSubmit = async (data: templateSchema) => {
    console.log(1);
    console.log(errors);
    if (watch("category") === "AUTHENTICATION") {
      const response = (await createAuthenticationTemplate(data)) as {
        data: apiRes;
      };
  ...
  return (
    <div className="flex h-screen gap-6 text-zinc-700">
      <div className="flex-1 h-full p-6 mb-6 overflow-y-scroll bg-white rounded-md shadow-lg">
        <form onSubmit={handleSubmit(onSubmit)} className="">
          ...
          {watch("category") === "AUTHENTICATION" && (
            <div className="space-y-4">
              <div className="border-input has-data-[state=checked]:border-primary/50 relative flex w-full items-start gap-2 rounded-md border p-4 shadow-xs outline-none">
                <Checkbox
                  checked={
                    !!watch("component.button.buttons")?.some(
                      (btn) => btn.type === "OTP"
                    )
                  }
                  onCheckedChange={(checked) => {
                    const buttons = watch("component.button.buttons") || [];
                    if (checked) {
                      if (!buttons.some((btn) => btn.type === "OTP")) {
                        buttonAppend({ type: "OTP", text: "Copy code" });
                      }
                    } else {
                      buttons.forEach((btn, idx: number) => {
                        if (btn.type === "OTP") buttonRemove(idx);
                      });
                    }
                  }}
              ...
              {watch("component.button.buttons.0.type") === "OTP" && (
                <div className="*:not-first:mt-2">
                  <Label>Button text</Label>
                  <Input
                    {...register("component.button.buttons.0.text")}
                    className="peer"
                    placeholder="Template name"
                  />
                  {watch("component.button.buttons.0.text") === "" && (
                    <p className="mt-1 text-sm text-red-500">
                      Enter button text
                    </p>
                  )}
              ...
              <div className="border-input has-data-[state=checked]:border-primary/50 relative flex w-full items-start gap-2 rounded-md border p-4 shadow-xs outline-none">
                <Checkbox
                  checked={
                    watch("component.body.add_security_recommendation") || false
                  }
                  onCheckedChange={(checked) =>
                    setValue(
                      "component.body.add_security_recommendation",
                      !!checked
                    )
                  }
                  className="order-1 after:absolute after:inset-0"
              ...
              <div className="border-input has-data-[state=checked]:border-primary/50 relative flex w-full items-start gap-2 rounded-md border p-4 shadow-xs outline-none">
                <Checkbox
                  // checked={!!watch("component.footer.expiry_enabled")}
                  onCheckedChange={(checked) => {
                    if (checked) {
                      setValue("component.footer.code_expiration_minutes", 10);
                      setValue(
                        "component.footer.text",
                        `This code expires in ${watch(
                          "component.footer.code_expiration_minutes"
                        )} minutes.`
                      );
                    } else {
                      setValue("component.footer.code_expiration_minutes", 10);
                      setValue("component.footer.text", "");
                    }
                  }}
                  className="order-1 after:absolute after:inset-0"
                />
                <div className="flex items-start gap-3 grow">
                  <TimerIcon />
                  <div className="grid gap-2">
                    <Label>Add expiry time for the code </Label>
                    <p className="text-xs text-muted-foreground">
              ...
              {watch("component.button.buttons.0.type") === "OTP" && (
                <div className="*:not-first:mt-2">
                  <Label>Expiry minute</Label>
                  <Input
                    {...register("component.footer.code_expiration_minutes", {
                      valueAsNumber: true,
                    })}
                    className="peer"
                    type="number"
                    placeholder="Expiry Minute"
                  />
                  {errors.component?.footer?.code_expiration_minutes && (
                    <p className="mt-1 text-sm text-red-500">
                      {errors.component.footer.code_expiration_minutes.message}
                    </p>
                  )}
                </div>
              )}
          ...
          {watch("category") !== "AUTHENTICATION" && (
            <>
                ...
                {watch("component.header.format") === "TEXT" && (
                  <div className="mt-2">
                    ...
                    <p className="mt-1 text-xs text-gray-500">
                      {watch("component.header.text")?.length || 0}/60
                      characters
                    </p>
              ...
              <div className="py-4 space-y-2 border-b">
                ...
                <p className="mt-1 text-xs text-gray-500">
                  Use {"{{1}}"}, {"{{2}}"} for variables. Format text with
                ...
                <div className="py-4 space-y-2 border-b">
                  {bodySampleFields.map((field, idx) => (
                    <div key={idx} className="">
                      ...
                      />
                      {errors.component?.body?.example
                        ?.body_text_named_params?.[idx]?.example && (
                        <p className="mt-1 text-sm text-red-500">
                          {
                            errors.component.body.example
                              .body_text_named_params[idx].example.message
                          }
                        </p>
                      )}
                    </div>
              ...
              <div className="py-4 space-y-2 border-b">
                <Button
                  onClick={() =>
                    buttonAppend({ type: "QUICK_REPLY", text: "" })
                  }
                  variant="outline"
                    ...
                    return (
                      <div key={idx} className="flex items-center space-x-2">
                          ...
                          <option
                            ...
                            disabled={
                              buttonFields.filter(
                                (_, i) =>
                                  watch(
                                    `component.button.buttons.${i}.type`
                                  ) === "URL"
                              ).length >= 2 &&
                        ...
                        <Input
                          ...
                          maxLength={
                            buttonType === "QUICK_REPLY" ? 20 : undefined
                          }
                          value={
                            buttonType === "COPY_CODE"
                              ? "Copy offer code"
                        ...
                        {buttonType === "COPY_CODE" && (
                          <Input
                            ...
                            className="w-full"
                            {...register(
                              `component.button.buttons.${idx}.example`
                            )}
