...
const Catalogue1Layout = ({
  ...
  return (
    <>
      <header
        style={{
          backgroundColor: lightenColor(data?.data?.backgroundColor, 0.85),
        }}
      ...
      >
        <div className="flex bg-cream items-center max-w-[80rem] w-full mx-auto justify-between gap-2 px-4 py-2 md:px-0 relative z-10">
          ...
          >
            <img src={data?.data?.logo?.url} alt="Logo" className=" h-13" />
          ...
          <div className="flex items-center gap-2 p-2 px-4 bg-black rounded-full">
            {cart?.length > 0 && (
              <NavLink
                to={`/catalogue/1/${userName}/cart`}
                className="border min-w-[25%] flex items-center justify-center rounded-md"
              >
                <div className="relative hover:text-blue-600">
                  <p className="bg-red-600 text-xs font-bold text-white absolute top-[-0.2rem] right-[-0.2rem] size-5 flex items-center justify-center rounded-full">
                    {cart.length}
                  </p>
      ...
      <div className="">
        {showScrollToTop && (
          <div
            onClick={scrollToTop}
            style={{
              backgroundColor: lightenColor(data?.data?.backgroundColor, -0.3),
            }}
            className="fixed text-white p-2 rounded-md shadow-md cursor-pointer bottom-2 right-2 z-[10]"
          >
            <IconArrowUp />
          </div>
        )}
      ...
      <footer
        className="border-t border-gray-300 mt-4 shadow-[10px_0px_10px_#808080]"
        style={{
          backgroundColor: lightenColor(data?.data?.backgroundColor, 0.85),
        }}
      >
        <div className="max-w-5xl py-4 mx-auto md:max-w-full md:pb-0">
          ...
          >
            <img src={data?.data?.logo?.url} alt="Logo" className="w-16 h-16" />
            <span className="text-lg font-semibold text-black">
              {data?.data?.name || "Profile Genie"}
            </span>
          </div>
          ...
          <ul className="flex flex-wrap justify-center gap-2 mt-2">
            <button
              onClick={scrollToTop}
              className="flex items-center justify-center px-4 py-2 text-sm font-semibold text-gray-900 transition rounded-md cursor-pointer hover:bg-gray-300/50 focus:outline-none"
            >
              Home
            </button>
            <Link
          ...
          <div className="flex flex-col items-center justify-center mt-2 md:border-t md:flex-row md:justify-between md:px-10 lg:px-16">
            <p className="my-4 text-sm text-center text-gray-900 ">
              Powered by{" "}
              <NavLink
                className={
                  "text-[#F43F5E] hover:text-[#F43F5E]/90 cursor-pointer font-semibold rounded-full text-sm hover:underline text-center "
                }
                to="/"
              >
                Profile Genie
