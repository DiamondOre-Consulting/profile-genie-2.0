...
const ProductItem = ({
  ...
  return (
    <div
      ...
      className={`p-1 mx-6 mb-3 select-none ${
        product.stock
          ? "opacity-100"
    ...
    >
      ...
      <div
        ...
        className="relative h-full shadow-md  rounded-xl"
      >
        <div
          style={{ backgroundColor: lightenColor(bgColor, 0.95) }}
          className="h-[200px]  rounded-xl flex justify-center items-center w-full"
        >
          <Swiper
            loop={true}
            autoplay={{
...
const ExploreProduct = ({
  ...
  cart: productDetail[];
  setCart: React.Dispatch<React.SetStateAction<productDetail[]>>;
  exploreProduct: { products: productDetail[] }[];
  bgColor: string;
}) => {
  return (
    <div className="py-10 overflow-hidden ">
      <h1
        className="mx-auto text-4xl font-semibold text-center  mf"
        data-aos="fade-left"
      >
        <SparklesText text="You may also like" />
      </h1>
      <div
        className="w-20 h-1 mx-auto mb-10 bg-dark md:w-60"
        data-aos="fade-left"
      ></div>
      <Marquee className="overflow-hidden h-fit" pauseOnHover={true}>
        <div className="flex gap-6 mx-3 overflow-hidden" data-aos="fade-up">
          {[
            ...(exploreProduct ?? []).flatMap(
              (item: { products: productDetail[] }) => item?.products ?? []
            ),
          ]
            .concat([
              ...(exploreProduct ?? []).flatMap(
