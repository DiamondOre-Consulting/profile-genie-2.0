...
const SideBar = ({ bgColor, cart, setCart, data: catalogueData }: { bgColor: string, cart: productDetail[], setCart: React.Dispatch<React.SetStateAction<productDetail[]>>, data: catalogueResponse }) => {
    ...
    return (
        <form onSubmit={handleSubmit(onSubmit)} className="relative p-2 pt-12 overflow-hidden border border-gray-300 rounded-md shadow-md lg:sticky lg:top-28">
            <div style={{ backgroundColor: lightenColor(bgColor, -0.35) }} className="absolute top-0 left-0 w-full py-2 text-center text-white ">
            ...
            <button disabled={isSubmitting} style={{ backgroundColor: lightenColor(bgColor, -0.3) }} type="submit" className="w-full px-4 py-2 mt-2 text-white rounded ">{isSubmitting ? <IconWhirl className="mx-auto animate-spin w-fit" /> : "Quote me!"}</button>
        </form>
    )
}
            ...
            <div className="flex">
                ...
                <div>
                    <div className="mb-4 text-base md:text-lg">
                        ...
                        <p className="text-sm leading-5 line-clamp-3">{product?.description}</p>
                    </div>
                    <div>
            ...
            <div className="container px-4 max-w-[80rem] mx-auto">
                {cart?.length > 0 ?
                    <div className="flex flex-col gap-6 py-14 md:py-24 lg:flex-row">
                        <div style={{ backgroundColor: lightenColor(bgColor, 0.95) }} className="w-full border border-gray-200 shadow-lg rounded-xl h-fit lg:w-2/3">
                    ...
                    <div className="flex flex-col items-center min-h-[90vh] justify-start p-4">
                        ...
                        <h2 style={{ color: lightenColor(bgColor, -0.3) }} className="mt-6 text-4xl font-bold text-blue-700">OOPS! Your Cart is Empty</h2>
