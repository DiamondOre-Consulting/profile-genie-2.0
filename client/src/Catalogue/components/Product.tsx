  ...
  return (
    <div
      onClick={(e) => {
        e.stopPropagation();
        navigate(
          `/catalogue/1/${userName}/product/${product?._id || product?.id}`
        );
      }}
      ...
      } shadow-md rounded-xl max-w-[22rem] mx-auto bg-white border border-gray-200 relative overflow-hidden`}
    >
      {!product?.stock && (
        <div className="absolute left-1/2 top-38 transform -translate-x-1/2 bg-black/90 text-white py-1 px-2 text-sm z-40 w-[150%] tracking-wide text-[1.3rem] uppercase -rotate-30">
          <p>Out Of Stock</p>
        </div>
      )}
      <div
        style={{ backgroundColor: lightenColor(bgColor, 0.95) }}
        className="relative h-full shadow-md bg-emerald-50 rounded-xl"
      >
        <div className="h-[200px] bg-emerald-50 rounded-xl flex justify-center items-center w-full">
          ...
          >
            {product.image.map(
              (image: { url: string; uniqueId: string }, index: number) => (
                <SwiperSlide key={index}>
                  <img
                    src={image?.url}
                    alt={`Slide ${index}`}
                    className="object-cover w-full mx-auto rounded-md h-80"
                  />
                </SwiperSlide>
              )
            )}
        ...
        <div className="p-3 text-start">
          <a href="#!">
            <h5
              ...
              className="text-[1.2rem] font-medium line-clamp-1"
            >
              {product.name}
            </h5>
          </a>
          <p className="line-clamp-3 text-sm leading-[20px]">
            {product?.description}
          </p>
          <div className="flex items-center w-full gap-4 mt-4 ">
            <div className="flex items-start flex-col min-w-[30%]">
              <h5
                style={{ color: lightenColor(bgColor, -0.3) }}
                className=" font-semibold leading-none text-[1.1rem]"
              >
                &#x20B9; {product.price.toLocaleString("en-IN")}
              </h5>
              {product?.stock && (
                <div className="text-[0.6rem] font-semibold bg-green-100 border border-green-600 text-green-600  px-2 mt-1 w-fit rounded-full">
                  In Stock
                </div>
              )}
            </div>
...
const Product = ({
  ...
  return (
    <section
      id="product"
      className="ezy__epgrid5 relative light py-14  bg-white dark:bg-[#0b1727] text-zinc-900   overflow-hidden "
    >
      {data?.uncategorisedProducts && (
        <div className="container py-10 lg:max-w-[80rem] xl:max-w-[90rem] px-4 mx-auto">
            ...
            {data?.uncategorisedProducts[0]?.products.map((product, i) => (
              <>
                {product?.productDetails && (
                  <div
                    className="col-span-12 px-2 my-4 sm:col-span-6 lg:col-span-4 xl:col-span-3"
                    key={i}
                  >
                    <ProductItem
                      userName={data?.data?.userName}
                      bgColor={data?.data?.backgroundColor}
      ...
      {data?.categorisedProducts &&
        data?.categorisedProducts.map((category, i) => (
          <>
            {category?.products?.length >= 1 && (
              <div
                key={i}
                className="container py-10 lg:max-w-[80rem] xl:max-w-[90rem] px-4 mx-auto"
              >
                <h2 className="my-6 text-center">
