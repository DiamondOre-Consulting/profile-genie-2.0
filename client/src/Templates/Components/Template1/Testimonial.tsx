...
const Testimonial = ({ testimonials }: { testimonials: portfolioResponse['contactData']['testimonial'] }) => {
            ...
            <div className="mx-auto flex max-w-container flex-col items-center gap-4 text-center sm:gap-8">
                ...
                <div className="relative flex w-full flex-col items-center justify-center overflow-hidden">
                    <Marquee pauseOnHover loop={0} direction="left" speed={30}>
                        {(testimonials?.testimonialList.length > 1) && Array.from({ length: 10 }).map(() => (
                            testimonials?.testimonialList?.map((testimonial, i) => (
                                <div key={i} className="w-[19rem] border text-white p-4 text-left h-full rounded-lg bg-[#101828] mx-1.5 ">
