                    ...
                    <div className="relative flex w-full flex-col items-center justify-center overflow-hidden pt-6">
                        <Marquee pauseOnHover loop={0} direction="left" speed={30}>
                            {Array.from({ length: 10 }).map(() => (
                                ((brands?.brandList?.length > 1) && brands?.brandList?.map((brand, i) => (
                                    <div key={i} className="  text-black text-left h-full rounded-lg  mx-1.5 ">
                                        <div className=" flex flex-col items-center gap-1 ">
                                            <img src={brand?.image?.url} className='rounded-md w-[5rem]' alt={brand?.brandName} />
