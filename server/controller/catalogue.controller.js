...
import {
  getCatalogueActiveMail,
  getCatalogueInactiveMail,
  getCataloguePaidMail,
  getQuotationMail,
  sendQuotationRes,
...
const createCatalogue = asyncHandler(async (req, res) => {
  ...
  const uniqueCatalogue = await Catalogue.findOne({
    $or: [{ userName: userName }, { catalogueOwner: catalogueOwner }],
  });
...
const editCatalogue = asyncHandler(async (req, res) => {
  ...
  if (uniqueCatalogue) {
    if (
      uniqueCatalogue.userName === userName &&
      uniqueCatalogue._id.toString() !== id
    ) {
      throw new AppError("Username already exists!", 400);
    }
  }
...
const addProduct = asyncHandler(async (req, res) => {
  ...
  const uniqueProduct = await CatalogueProduct.findOne({
    owner: ownerId,
    HSNCode,
  });
  ...
  image?.forEach((image) => {
    let existingImage = product?.image?.find(
      (b) => b.uniqueId === image.uniqueId
    );
    if (!existingImage) {
      const uploadedFile = productImages.find(
        (uf) => uf.uniqueId === image.uniqueId
      );
      if (uploadedFile) {
        product.image.push({
          ...image,
      ...
      const uploadedFile = brandImages.find(
        (uf) => uf.uniqueId === existingImage.uniqueId
      );
      if (uploadedFile) {
...
const editProduct = asyncHandler(async (req, res) => {
  ...
  product.image = product?.image?.filter((productData) => {
    const isExistingData = image.some(
  ...
  const productWithoutCategory = await Catalogue.aggregate([
    ...
    },
    {
      $match: {
        $expr: {
          $eq: [
            { $size: { $ifNull: ["$filteredCategories", []] } },
            { $size: { $ifNull: ["$productDetails.category", []] } },
          ],
        },
      },
    },
      ...
      $match: {
        ...(search && {
          $or: [
            ...
            {
              "ownerDetails.authAccountDetails.fullName": {
                $regex: search,
                $options: "i",
              },
            },
            {
              "ownerDetails.authAccountDetails.email": {
                $regex: search,
                $options: "i",
              },
            },
...
const getSingleCatalogue = asyncHandler(async (req, res) => {
  ...
  const catalogue = await Catalogue.findOne({
    $or: [{ userName: userName }, { catalogueOwner: catalogueOwner?._id }],
  })
    .populate({
      path: "catalogueOwner",
      populate: {
...
const sendQuotation = asyncHandler(async (req, res) => {
  ...
  const { subject, message } = getQuotationMail(
    req.body,
    products,
    catalogue?.name,
    catalogue?.logo?.url
  ...
  await sendMail(
    catalogue?.catalogueOwner?.authAccount?.email,
    subject,
    message
  );

  const { subject: quotationResSubject, message: quotationResMessage } =
    sendQuotationRes(req.body, products, catalogue?.name, catalogue?.logo?.url);
...
const deleteCatalogue = asyncHandler(async (req, res) => {
  ...
  if (catalogue.product) {
    ...
    const catalogueProducts = await CatalogueProduct.find({
      _id: { $in: productIds },
    });
  ...
  if (catalogue.catalogueOwner)
    await CatalogueOwner.findByIdAndDelete(catalogue.catalogueOwner);
  if (catalogue.metaDetails)
    await MetaData.findByIdAndDelete(catalogue.metaDetails);
...
const updateStatusActive = asyncHandler(async (req, res) => {
  ...
  const { subject, message } = getCatalogueInactiveMail(
    catalogue?.catalogueOwner?.authAccount?.fullName,
    catalogue?.userName,
    catalogue?.name
  );
  const { subject: activeSubject, message: activeMessage } =
    getCatalogueActiveMail(
      catalogue?.catalogueOwner?.authAccount?.fullName,
      catalogue?.userName,
      catalogue?.name
...
const updateStatusPaid = asyncHandler(async (req, res) => {
  ...
  catalogue.paidDate = catalogue.isPaid
    ? new Date().toISOString().split("T")[0] + "T00:00:00.000Z"
    : catalogue.paidDate;

  await catalogue.save();

  const { confirmMailSubject, confirmMailMessage } = getCataloguePaidMail(
    catalogue?.catalogueOwner?.authAccount?.fullName,
    catalogue?.userName,
    catalogue?.name
  );
...
const editCategory = asyncHandler(async (req, res) => {
  ...
  const selectedCategory = catalogue.category.find(
  ...
  const uniqueCategory = catalogue.category.find(
    (cat) => cat.text === category.name
  );
  if (uniqueCategory) {
    throw new AppError("Category already exists!", 400);
  }

  catalogue.product = await Promise.all(
    catalogue.product.map(async (productId) => {
